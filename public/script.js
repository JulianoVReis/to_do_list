...
document.addEventListener("click", (event) => {
	...
	const clicouEmEditarTexto = event.target.classList.contains("editar-texto") || modalEditar.contains(event.target);

	for (const modal of modais) {
		if (modal.classList.contains("mostrar") && !modal.contains(event.target) && !event.target.closest(".modal-1, .modal-2, .modal-3") && !clicouEmEditarTexto) {
			modal.classList.remove("mostrar");
			setTimeout(() => (modal.style.display = "none"), 300);
		}
	}

	if (!modalEditar.classList.contains("hidden") && !modalEditar.contains(event.target) && !event.target.classList.contains("editar-texto")) {
		modalEditar.classList.add("hidden");
	}
});
